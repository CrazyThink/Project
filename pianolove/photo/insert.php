...
<?
	...
	if ($mode=="modify")
	{
		...
		$sql = "update $table set subject='$subject' where num=$num";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	else
	{

		$sql = "insert into $table (id, nick, subject, regist_day, ";
		$sql .= " file_name_0, file_copied_0) ";
		$sql .= "values('$userid', '$usernick', '$subject', '$regist_day', ";
	...
	echo "
	   <script>
	    location.href = 'photo.php?table=$table&page=$page';
	   </script>
	";
?>
